...
async def generate_form_background(company_id: str):
    ...
    filtered_fields = [{k: v for k, v in field.items() if k != "rect"} for field in FORM_125_FIELDS]
    ...
    for batch_number in range(1, total_batches + 1):
        ...
        batch_results = await process_field_batch(company_md, batch_fields, batch_number, total_batches)
        all_results.update(batch_results)
    
    all_results = await validate_generated_fields(company_md, all_results, FORM_125_FIELDS)

    with open(f"../output/{company_id}.json", "w") as json_file:
...
async def generate_form(company_id: str, all_results: dict):
    ...
    async with httpx.AsyncClient() as client:
        try:
            pdf_id = company_id.replace("/", "_") if "/" in company_id else company_id
            ...
            async with httpx.AsyncClient() as client:
                with open(f"../output/{filename}", "rb") as pdf_file:
                    pdf_blob = pdf_file.read()
                    pdf_blob_base64 = base64.b64encode(pdf_blob).decode('utf-8')
                    response = await client.post(
                        "http://localhost:3000/api/generate-pdf",
                        json={"companyId": company_id, "pdfBlob": pdf_blob_base64}
                    )
                    response.raise_for_status()
...
async def process_field_batch(company_md: str, fields: list, batch_number: int, total_batches: int) -> dict:
    """Process a batch of form fields and return their values."""
    prompt = fill_form_fields_prompt(company_md, fields)
    
    try:
        response = openai_completions(
            prompt=prompt,
...
async def validate_generated_fields(company_md: str, generated_fields: dict, fields: list):
    prompt = validate_generated_fields_prompt(company_md, generated_fields, fields)
    response = openai_completions(
        prompt=prompt,
        model="gpt-4o-mini-2024-07-18",
        response_format={"type": "json_object"}
