...
async def refine_form_background(company_id:str, refine_task:str):
    with open(f"{company_id}.json", "r") as json_file:
...
def refine_form(refine_task:str, generated_form:dict):
    fields_of_interest_prompt = identify_fields_to_refine_prompt(refine_task, generated_form)
    ...
    updated_form_prompt = update_fields_prompt(refine_task, generated_form, fields_of_interest)
    updated_form = openai_completions(
        prompt=updated_form_prompt,
        model="gpt-4o-mini-2024-07-18",
        response_format={"type": "json_object"}
    )
    return updated_form